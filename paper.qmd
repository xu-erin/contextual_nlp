...
This document is intended for technical stakeholders to reference when reviewing the proof-of-concept implementation of a context-aware natural language processing (NLP) enrichment and retrieval workflow for pharmaceutical deviation records. It describes first the problem statement, objectives, data flow, and storage organization, inputs, outputs produced by the pipeline, and the technical dependencies required to run the workflow. It also explains how to use the semantic-search layer to build retrieval cohorts for specific business questions, and documents the hard-coded
statistics, thresholds, and heuristics configured in the reporting layer—so that every analyst-set value can be audited and adjusted. For any unresolved curiosity, there is also a section outlining the justification of this methodology as well as the conceptual breakdowns of the scientific concepts behind this technology. Finally, future work and next steps are outlined.
...
Suggestions of these practices will be outlined in the final section.
...
+-------------------------------------+------------------------------------------------------+
| Vector Search endpoint              | `deviation-retrieval-vs` (type `STANDARD`)           |
+-------------------------------------+------------------------------------------------------+

## Running the Code

The workflow is a set of Databricks notebooks that must be run in **dependency
order**: each writes Delta tables or vector indexes that later notebooks read. Run
the cells top-to-bottom within each notebook. `build_reference_glossary` is the
...
A purely semantic index is insufficient, because dense embeddings capture meaning but
miss exact matches, whereas BM25 recovers them. This is decisive for deviation records
...
(about 601M tokens) reinforces this: holding questions and evidence fixed while growing
...
re-encoding and only encodes the short candidate-label strings.

The first strategy defines 18 candidate topics from GxP quality vocabulary
(inadvertent unblinding, GMP manufacturing deviation, stability OOS). Each
...
Information, and quality is compared via outlier rate and topic diversity, which is the
...
I had many wonderful conversations along the way with Mingxi Song, Kevin Cheng, Minnan Xu, Neda Hassanpour, Nellie Holger, and everyone I met across MA Bio Ops, DD&T, and over lunch. Finally, I would especially like to thank Xuemei Li and Yunliang Cai for their
mentorship. They gave me a great deal of perspective. 

This project was, for me, far more than a technical deliverable. I came away with a more independent (agentic, if you will) problem-solving workflow, a clearer sense of how to set goals and learned what it means to build within data-governance constraints. I was never drawn to NLP, yet somehow now, I cannot stop thinking about it. Retrieval-system design and the Databricks documentation page plague my nightly visions. I realized I was interested in something in the domain of NLP after all, namely, optimal guarantees for topic models with an unknown number of topics using NMF. I will head into my final year of undergraduate study at the University of Toronto. My primary interest remains in applying machine learning and multivariate statistics to medicine and biology; functional analysis and representation learning. If you would like to continue the conversation, please feel free to reach out to me on LinkedIn.
